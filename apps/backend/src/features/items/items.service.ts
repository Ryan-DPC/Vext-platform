...
export class ItemsService {
    ...
    try {
      ...
      const items = result.resources.map((resource: any, index: number) => {
        ...
        return {
          ...
          rarity: rarity,
          price:
            (rarity === 'common' ? 100 : rarity === 'rare' ? 250 : rarity === 'epic' ? 500 : 1000) *
            (itemType === 'banner' ? 2 : 1),
  ...
  static async getUserItems(userId: string) {
    const docs = await UserItemModel.find({ user_id: userId }).populate('item_id').lean();
    return docs
      .map((d: any) => {
  ...
  static async equipItem(userId: string, itemId: string) {
    ...
    const allUserItems = await UserItemModel.find({ user_id: userId }).populate('item_id').lean();
    ...
    for (const userItem of allUserItems) {
      ...
      if (currentItem && currentItem.item_type === (item as any).item_type) {
        await UserItemModel.updateOne({ _id: userItem._id }, { $set: { is_equipped: false } });
      }
    }
    ...
    try {
      ...
      if (userId) {
        ...
        return items.map((item: any) => ({
          ...
          equipped:
            userItems.find((ui: any) => ui.item?.id === item.id && ui.is_equipped) !== undefined,
        }));
      }
  ...
  static async purchaseItem(userId: string, itemId: string) {
    try {
      ...
      if ((item as any).is_archived) {
        throw new Error(
          'Cet item est "Legacy" (archivé) et n\'est plus disponible à l\'achat dans la boutique officielle.'
        );
      }
      ...
      if ((user.tokens || 0) < (item.price || 0)) {
        throw new Error(
          `Solde insuffisant. Vous avez ${user.tokens} tokens, prix: ${item.price} tokens`
        );
      }
  ...
  static async archiveItem(itemId: string) {
    try {
      ...
      return {
        success: true,
        message: 'Item archivé avec succès. Les propriétaires actuels le conservent.',
      };
    } catch (error) {
