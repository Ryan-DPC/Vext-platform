...
// Determine if input is likely an email (contains @)
const isEmailInput = computed(() => email.value.includes('@'));

// Validate email format only if it looks like an email
const isEmailValid = computed(() => {
  if (!isEmailInput.value) return true; // It's a username, so it's valid format-wise
...
const handleLogin = async () => {
  ...
  if (isEmailInput.value && !isEmailValid.value) {
      ...
      <form @submit.prevent="handleLogin">
        <div class="form-group">
          ...
          >
            ...
            />
            <i v-if="email && isEmailValid" class="fas fa-check validation-icon valid"></i>
            <i
              v-else-if="email && !isEmailValid && isEmailInput"
...
.validation-icon {
  position: absolute;
  right: 45px; /* Position left of the eye icon or end of input */
  top: 50%;
  transform: translateY(-50%);
  font-size: 0.9rem;
}
