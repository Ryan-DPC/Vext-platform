...
const saveChanges = async () => {
  ...
  if (
    ...
    JSON.stringify(userStore.user.notification_preferences)
  ) {
  ...
  if (storedPaths) {
    ...
    if ((window as any).__TAURI__) {
      try {
        ...
        const defaultPath =
          localStorage.getItem('etherInstallPath') ||
          'C:\\Users\\Default\\Documents\\Vext-platform\\Vext';
  ...
  <div class="settings-page">
    <div class="settings-container">
      ...
      <div class="settings-content">
        <h1 class="page-title">
          {{ t('settings.title', { tab: t(`settings.tabs.${activeTab}`) }) }}
        </h1>
            ...
            >
              ...
              <div class="theme-info">
                ...
                <div v-if="theme.requiredItem" class="theme-req">
                  <i class="fas fa-lock" style="font-size: 0.8em; margin-right: 5px"></i>
                  {{ theme.requiredItem }}
                </div>
              </div>
            </div>
          ...
          <div class="plugins-list">
            ...
            <div v-for="(plugin, index) in themeStore.plugins" :key="index" class="plugin-item">
              ...
              <div class="plugin-actions">
                <button class="remove-btn" @click="themeStore.removePlugin(index)">
                  {{ t('settings.appearance.remove') }}
                </button>
              </div>
            </div>
            <button class="add-plugin-btn" @click="addNewPlugin()">
              {{ t('settings.appearance.install_plugin') }}
            </button>
          </div>
        </div>
