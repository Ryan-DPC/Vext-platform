const SERVER_URL = 'wss://vext-game-server-1mgy.onrender.com/ws';
...
async function simulate() {
    ...
    // Capture Guest ID from the server's broadcast
    const guestJoinedParams = (joinResult[0] as any).data;
    const guestId = guestJoinedParams.playerId;
    console.log(`👥 Guest joined successfully. ID: ${guestId} (Requested: ${playerB.userId})`);
    ...
    wsA.send(
        JSON.stringify({
            ...
            data: {
                ...
                targetId: guestId, // Use captured ID
                damage: 10,
                manaCost: 0,
                isArea: false,
            },
    ...
    wsA.send(
        JSON.stringify({
            ...
            data: { nextTurnId: guestId }, // Use captured ID
        })
    );
    ...
    const rewardWave = await waitForMessage(wsB, 'aether-strike:wave-started');
